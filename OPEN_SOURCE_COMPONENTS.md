...
Nicevideodown 的源码使用 MIT License。Windows 便携版同时分发下列第三方组件，各组件继续遵循自己的许可证。
...
便携版包含 Electron 与 Chromium 自带的许可证文件。GPL 和 Unlicense 全文位于 `licenses` 目录。qrcode-generator 的 MIT 声明见 `THIRD_PARTY_NOTICES.md`。

本项目不主张对第三方组件拥有权利。第三方组件的商标、版权和专利权归各自权利人所有。
